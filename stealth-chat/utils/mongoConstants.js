// MongoDB-only user utilities - no localStorage dependency

// Get users from MongoDB API
...
export const findUserByUsernameDB = async (username) => {
  try {
    const response = await fetch(`/api/users/find?username=${encodeURIComponent(username)}`);
    if (response.ok) {
      const data = await response.json();
...
// Create default demo users in MongoDB if none exist
...
// File upload constraints (unchanged)
...
// Flipkart categories for homepage (unchanged)
